...
class Monsters extends Component {
  ...
  updateInput = ({target: {value}}) => {
    this.setState({ userInput: value, filtered: this.state.monsters.filter(({name}) => name.toLowerCase().includes(value.toLowerCase()))});
  }
  ...
  render() {
    return (
      <div className="Monsters">
        <h1>컴포넌트 재사용 연습!</h1>
        <SearchBox handleChange={this.updateInput} />
        <CardList monsters={this.state.filtered} />
      </div>
    );
  }
}
