...
class Monsters extends Component {
  ...
  render() {
    let result = this.state.monsters.filter((monsters)=>{
      return monsters.name.toLowerCase().includes(this.state.userInput.toLowerCase()); 
    })
    return (
      <div className="Monsters">
        <h1>컴포넌트 재사용 연습!</h1>
        <SearchBox handleChange={this.handleChange}></SearchBox>        
        <CardList  monsters={result}></CardList>
      </div>
    );
  }
}
