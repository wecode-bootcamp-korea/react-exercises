...
class Monsters extends Component {
  ...
  componentDidMount() {
    fetch("https://jsonplaceholder.typicode.com/users")
      .then((res) => res.json())
      .then((res) => {
        let originMonsters = res;
        this.setState({ ...this.state, monsters: originMonsters });
      });
  }
  ...
  handleFilter = (userInput) => {
    let result = this.state.monsters.filter(
      (monster, id) => monster.name.indexOf(userInput) != -1
    );
  ...
  render() {
    // 필터링 로직
    const monster = this.handleFilter(this.state.userInput);

    return (
      <div className="Monsters">
        <h1>컴포넌트 재사용 연습!</h1>
        <SearchBox data={this.state} handleChange={this.handleChange} />
